---
title: "Demo: modelling spatial correlation in damage"
...
The spatial ordinal model takes about 8 minutes in a PC with characteristics: Intel(R) Xeon (R) W-2112 CPU Processor @ 3.60GHz; 32GB of RAM; Windows 10 64-bit. 
...
From Figure 7, we estimate a spatial range for the shared spatial field of about 15 km, while those specific to Building category 1 and 2 are found to be about 2-2.5km. The effect of these are shown in the estimated spatial fields around the captial of Port-au-Prince in Figures 8(b)-(c) and 9(b)-(c).  
...
loc.df <- data.frame("Easting" = c(782000, 782000), "Northing"  = c(2053000, 2045000), "Location"  = c("Site 1", "Site 2"))
...
In Figure 10(a), we chose two locations (Site 1 and 2) to illustrate the effect of the Building category 1 specific spatial field on its ordinal probability distributions. Nearer Site 2, higher latent variable mean values lead to a shift in the probability density to the right. This in turn leads to higher exceedance probabilities for higher damage states. The converse holds for locations near Site 1. This can also be seen in the maps of exceedance probabilities in Figure 11. 
...
# ======== Run and change to demo_field_sim.RData later.

# load(file = "D:/Documents/Proj_Damage_Spatial_Corr/Data/new_field_sim.RData")
...
temp.time <- proc.time()[3]
pgaspat_lv <- lv_sim(model = "PGA-spatial", data = oq_sim, fieldsim = field.sim,
                     field1sim = NULL, field2sim = NULL, slope1 = beta1, slope2 = beta2,
                     shared_sill = field.tau2 + field.sigma2)
...
save(nonspat.al1, nonspat.al2, pgaspat.al1, pgaspat.al2, damagespat.al1, damagespat.al2, 
     file = "D:/Documents/Proj_Damage_Spatial_Corr/Data/new_al_sim.RData")

# ======== Run and change to demo_al_sim.RData later.

# load(file = "D:/Documents/Proj_Damage_Spatial_Corr/Data/new_al_sim.RData")
...
for (i in 1:length(al_vec)){
  ...
  # Non-spatial model - not as a submodel:
  nonspat_nonsubmodel.ar[i] <- sum((nonspat_nonsubmodel.al1 + nonspat_nonsubmodel.al2) > al_vec[i])/no_ses
  nonspat_nonsubmodel.ar_ex1[i] <- sum(nonspat_nonsubmodel.al1 > al_vec[i])/no_ses
  nonspat_nonsubmodel.ar_ex2[i] <- sum(nonspat_nonsubmodel.al2 > al_vec[i])/no_ses
}
